...
static CdoLogLevel parse_log_level(const char* str) {
    ...
    return CDO_LOG_INFO; // default on unrecognized
}
...
static CdoColorMode parse_color_mode(const char* str) {
    ...
    return CDO_COLOR_AUTO; // default on unrecognized
}
...
int cdo_cli_parse(int argc, char** argv, CdoOptions* opts) {
    ...
    opts->command   = CDO_CMD_HELP; // default if no command given

    if (argc < 2) {
        return 0; // no args → CDO_CMD_HELP
    }

    // We use static arrays for positional args to avoid heap allocation.
    // Max 64 positional args and 64 rest args should be more than enough.
    ...
    for (int i = 1; i < argc; i++) {
        ...
        // After "--", everything goes to argv_rest
        if (rest_mode) {
        ...
        if (is_option(arg)) {
            ...
            if (strncmp(arg, "-j", 2) == 0) {
                if (arg[2] != '\0') {
                    // -jN (number directly after -j)
                    opts->jobs = parse_int(arg + 2);
                } else {
                    // -j N (number in next arg)
                    if (i + 1 < argc) {
                        opts->jobs = parse_int(argv[++i]);
                    }
                }
...
void cdo_cli_print_help(CdoCommand cmd, FILE* out) {
    ...
    case CDO_CMD_UNKNOWN:
        // General help — summary of available commands
        fprintf(out,

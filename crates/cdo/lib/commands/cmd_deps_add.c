...
static TomlTable* manifest_get_or_create_section(TomlTable* root, const char* section) {
    ...
    if (val != NULL) {
        ...
        cdo_log_error("[%s] exists but is not a table", section);
        return NULL;
    }
...
static int append_string_array(TomlTable* tbl, const char* key, char* const* strings, int count) {
    if (count <= 0) return 0;
...
static int deps_persist_build_metadata(TomlTable* dep_inline_tbl, const CatalogResolveResult* resolved) {
    if (append_string_array(dep_inline_tbl, "include_dirs", resolved->include_dirs, resolved->include_dir_count) != 0)
        return 1;
    if (append_string_array(dep_inline_tbl, "link_libs", resolved->link_libs, resolved->link_lib_count) != 0)
        return 1;
    if (append_string_array(dep_inline_tbl, "defines", resolved->defines, resolved->define_count) != 0)
        return 1;
    return 0;
}

/// Add a catalog-resolved dependency entry with full build metadata to the deps table.
static int deps_add_catalog_entry(TomlTable* deps, const char* name, const CatalogResolveResult* resolved) {
...
static void parse_name_version(const char* input, char* name_buf, size_t name_size, char* version_buf, size_t version_size) {
...
int cmd_deps_add(const CliParseResult* result, void* ctx) {
    ...
    /* Extract --dev flag from parsed args */
    bool dev_mode = cli_arg_get_bool(result, "dev");

    /* Extract --version constraint if provided */
    const char* explicit_version = cli_arg_get_str(result, "version");

    /* Positional args are the package names (e.g., "mylib" or "mylib@1.0") */
    if (result->positional_count < 1) {
        cdo_log_error("Usage: cdo deps add <package[@version]> [--dev]");
        return 1;
    }
    ...
    if (cat_rc != 0) {
        cdo_log_warn("Failed to load catalog â€” falling back to registry lookup");
    }
    ...
    if (plat_rc != 0) {
        cdo_log_warn("Could not detect platform â€” catalog resolution may fail");
    }
    ...
    for (int i = 0; i < result->positional_count; i++) {
        const char* arg = result->positional_values[i];

        /* Parse name@version suffix */
        char pkg_name[128];
        char version_constraint[64];
        parse_name_version(arg, pkg_name, sizeof(pkg_name), version_constraint, sizeof(version_constraint));

        /* If --version was explicitly provided, use it over inline @version */
        if (explicit_version && explicit_version[0] != '\0') {
            snprintf(version_constraint, sizeof(version_constraint), "%s", explicit_version);
        }

        /* Check for conflict: --dev but already in [dependencies] */
        if (dev_mode && cmd_deps_has(deps, pkg_name)) {
            cdo_log_error("'%s' already exists in [dependencies] â€” cannot add as dev-dependency", pkg_name);
            rc_result = 1;
            continue;
        }
        ...
        if (cmd_deps_has(target_deps, pkg_name)) {
            cdo_log_info("'%s' is already in %s â€” skipping", pkg_name, dev_mode ? "dev-dependencies" : "dependencies");
            continue;
        }
        ...
        int resolve_rc = catalog_resolve_package(&catalog, pkg_name, constraint, &platform, &resolved);

        if (resolve_rc == 0) {
            /* Catalog resolution succeeded â€” add with full metadata */
            int rc = deps_add_catalog_entry(target_deps, pkg_name, &resolved);
            if (rc != 0) {
                cdo_log_error("Failed to add '%s' to manifest", pkg_name);
                rc_result = 1;
                catalog_resolve_result_free(&resolved);
                continue;
            }

            cdo_log_info("Added '%s' version %s (from catalog)%s", pkg_name, resolved.version, dev_mode ? " [dev]" : "");
            catalog_resolve_result_free(&resolved);
        } else {
            /* Catalog resolution failed â€” fall back to registry */
            cdo_log_warn("Package '%s' not found in catalog â€” using registry fallback", pkg_name);
            ...
            if (rc != 0) {
                cdo_log_error("Failed to resolve package '%s'", pkg_name);
                rc_result = 1;
            ...
            cdo_log_info("Added '%s' version %s%s", pkg_name, version, dev_mode ? " [dev]" : "");
            dep_resolved_free(&resolved_dep);
        }
    }

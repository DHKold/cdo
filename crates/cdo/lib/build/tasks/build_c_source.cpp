// build_c_source.cpp — BuildCSource implementation.
...
BuildCSource::BuildCSource(Config config, bool forced)
...
const std::vector<const Artifact*>& BuildCSource::inputs() const {
    return inputs_cache_;
}

const std::vector<const Artifact*>& BuildCSource::outputs() const {
    return outputs_cache_;
}

const Artifact& BuildCSource::primaryOutput() const {
    return output_artifact_;
}

const TaskCondition& BuildCSource::condition() const {
    return condition_;
}

int BuildCSource::execute() {
    // TODO: Spawn compiler process via PAL when process API is integrated.
    ...
    //                   -std=<c_standard> -I<include_paths> -D<defines> <extra_flags>
    cdo_log_debug("BuildCSource::execute() stub for %s", config_.source_path.c_str());
    return 0;
}

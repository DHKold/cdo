/**
 * build_shared_library.h - Concrete task for linking object files into a shared library.
 *
 * BuildSharedLibrary links .o files into a .dll (Windows) or .so (POSIX)
 * shared library using the detected linker.
 *
 * Part of the cdo::build pipeline refactor.
 */
#ifndef CDO_BUILD_TASKS_BUILD_SHARED_LIBRARY_H
#define CDO_BUILD_TASKS_BUILD_SHARED_LIBRARY_H
...
/// Links object files (.o) and libraries into a shared library (.dll/.so).
/// Produces a single output: the shared library file.
...
#endif // CDO_BUILD_TASKS_BUILD_SHARED_LIBRARY_H

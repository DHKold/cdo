...
TEST(workspace_load_valid) {
    ...
    // The workspace cdo.toml has members = ["crates/cdo", "crates/cdo_pbt"]
    TEST_ASSERT(ws.crate_count >= 2);

    // Verify known crate names exist
    int found_cdo = 0;
    int found_cdo_pbt = 0;
    for (int i = 0; i < ws.crate_count; i++) {
        if (strcmp(ws.crates[i].name, "cdo") == 0) {
            found_cdo = 1;
            TEST_ASSERT(ws.crates[i].type == CRATE_EXECUTABLE);
        }
        if (strcmp(ws.crates[i].name, "cdo_pbt") == 0) {
            found_cdo_pbt = 1;
            TEST_ASSERT(ws.crates[i].type == CRATE_TEST);
        }
    }
    TEST_ASSERT(found_cdo);
    TEST_ASSERT(found_cdo_pbt);
...
TEST(workspace_resolve_no_cycles) {
    ...
    // Resolve build order for "cdo_pbt" (which depends on "cdo")
    const char* names[] = {"cdo_pbt"};
    ...
    // cdo_pbt depends on cdo.
    int cdo_idx_in_order = -1;
    int cdo_pbt_idx_in_order = -1;
    for (int i = 0; i < ws.build_order_count; i++) {
        int crate_idx = ws.build_order[i];
        if (strcmp(ws.crates[crate_idx].name, "cdo") == 0) {
            cdo_idx_in_order = i;
        }
        if (strcmp(ws.crates[crate_idx].name, "cdo_pbt") == 0) {
            cdo_pbt_idx_in_order = i;
        }
    }
    // Both should be present in the build order
    TEST_ASSERT(cdo_idx_in_order >= 0);
    TEST_ASSERT(cdo_pbt_idx_in_order >= 0);
    // "cdo" must come before "cdo_pbt"
    TEST_ASSERT(cdo_idx_in_order < cdo_pbt_idx_in_order);
